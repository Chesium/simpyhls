...
#
# Responsibilities:
# - discover which raw flooded regions are grounded by scanning the canvas
# - compact sparse raw region ids into continuous solver row indices
# - write node0/node1 for every indexed two-terminal component
#
# Interface notes:
# - Ground cells are discovered directly from fetchCell(i, j), not from the
#   indexed component store.
# - Indexed component traversal is limited to true two-terminal components.
#
# Compaction policy:
# - every ground-connected raw region -> 255
            ...
            if u8_raw0 != 0:
                for u8_scan_y in range(grid_height):
                    for u8_scan_x in range(grid_width):
                        u16_ground_cell = fetchCell(i=u8_scan_x, j=u8_scan_y)
                        if is_ground_cell_comb(cell=u16_ground_cell):
                            u2_ground_rot = get_cell_rotation_comb(cell=u16_ground_cell)
                            u8_ground_term_x = get_nxt_i_comb(i=u8_scan_x, d=get_opp_dir_comb(d=u2_ground_rot))
                            u8_ground_term_y = get_nxt_j_comb(j=u8_scan_y, d=get_opp_dir_comb(d=u2_ground_rot))
                            if u8_ground_term_x < grid_width and u8_ground_term_y < grid_height:
                                u8_ground_region = fetchR(i=u8_ground_term_x, j=u8_ground_term_y)
                        ...
                        if u8_region != 0:
                            for u8_prev_y in range(grid_height):
                                for u8_prev_x in range(grid_width):
                                    u16_ground_cell = fetchCell(i=u8_prev_x, j=u8_prev_y)
                                    if is_ground_cell_comb(cell=u16_ground_cell):
                                        u2_ground_rot = get_cell_rotation_comb(cell=u16_ground_cell)
                                        u8_ground_term_x = get_nxt_i_comb(i=u8_prev_x, d=get_opp_dir_comb(d=u2_ground_rot))
                                        u8_ground_term_y = get_nxt_j_comb(j=u8_prev_y, d=get_opp_dir_comb(d=u2_ground_rot))
                                        if u8_ground_term_x < grid_width and u8_ground_term_y < grid_height:
                                            u8_ground_region = fetchR(i=u8_ground_term_x, j=u8_ground_term_y)
                            ...
                            for u8_prev_y in range(grid_height):
                                for u8_prev_x in range(grid_width):
                                    if (
                                        ...
                                        u8_prev_region = fetchR(i=u8_prev_x, j=u8_prev_y)
